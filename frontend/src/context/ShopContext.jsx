...
const ShopContextProvider = (props) => {
    ...
    useEffect(() => {
        // Check if token is not set in the state but exists in localStorage
        const storedToken = localStorage.getItem('token');
        
        if (storedToken && !token) {
            setToken(storedToken); // Set the token in the state
        }
    }, [token]); // This will run whenever the token changes
    
    useEffect(() => {
        // Only fetch cart data when token is set
        if (token) {
            getUserCart();
        }
    }, [token]); // This will run whenever token change
